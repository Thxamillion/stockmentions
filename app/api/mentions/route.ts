...
export async function GET(request: NextRequest) {
  try {
    ...
    let subredditCondition = ''
    if (subreddit !== 'all') {
      subredditCondition = `AND subreddit = '${subreddit}'`
    }
    ...
    const query = `
      WITH ticker_stats AS (
        ...
        FROM mentions
        WHERE ${timeCondition} ${subredditCondition}
        GROUP BY ticker
      ),
      ranked_tickers AS (
        SELECT
    ...
    const tickers = result.rows.map((row, index) => ({
      rank: index + 1, // Re-rank from 1-10 after filtering
      ticker: row.ticker,
      posts: parseInt(row.posts),
      comments: parseInt(row.comments),
      change: Math.random() * 40 - 20 // Mock change percentage for now
    }))

    return NextResponse.json({ tickers, timeRange, subreddit })
  } catch (error) {
